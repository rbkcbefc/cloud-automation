...
module "ecs_test_auto_mock_email_svc_tg" {
  ...
  tg_name = "ecs-mock-email-service"
  tg_port = 80
  tg_protocol = "HTTP"
  tg_health_check_path = "/mock-email-service/index.jsp"
...
terraform {
  backend "s3" {
    key = "envs/aws/infra/ecs/nightly/test-auto/target-group/mock-email-service/terraform.state"
  }
}