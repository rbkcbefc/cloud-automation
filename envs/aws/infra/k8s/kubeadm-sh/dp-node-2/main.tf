...
module "k8s_kubeadm_sh_dp_node_2" {
...
resource "null_resource" "run-ansible-playbook" {
  depends_on    = [module.k8s_kubeadm_sh_dp_node_2, time_sleep.wait-for-cloud-init-completion]
...
terraform {
  backend "s3" {
    key            = "envs/aws/infra/k8s/kubeadm-sh/dp-node-2/terraform.state"
  }
}